...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetServices(struct soap* soap, struct _tds__GetServices *tds__GetServices, struct _tds__GetServicesResponse *tds__GetServicesResponse)
{
    ...
    /*-----------------------------------------------------------------------------
     *  NVT需要实猿device,events,media,PTZ,Imaging,Video Analytics, Device IO
     *-----------------------------------------------------------------------------*/
...
//    const char * service_caplties[]={device, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
    ...
    /*-----------------------------------------------------------------------------
     *  若为"http://www.onvif.org/ver10/media/wsdl" 则对帿测试工具接收media service
     *  信息，若为event/wsdl则只能接收event信息,除非在客户端include 性能=true的情儿
     *  下可以接撿和测试项目的矛盾怎么庿会在传入参数中有所体现䷿
     *-----------------------------------------------------------------------------*/
    ...
    for(i=0;NULL!=services[i];i++)
    {
        ...
        else
        {
            ...
            /*-----------------------------------------------------------------------------
             *  需要对其域吿提供功能的true或者false XML格式返回,如下
             *-----------------------------------------------------------------------------*/
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetDeviceInformation(struct soap* soap, struct _tds__GetDeviceInformation *tds__GetDeviceInformation, struct _tds__GetDeviceInformationResponse *tds__GetDeviceInformationResponse)
{
     ...
     *-----------------------------------------------------------------------------*/
    tds__GetDeviceInformationResponse->Manufacturer =  "EyeCloud";
    tds__GetDeviceInformationResponse->Model =  "OpenNCC";
    tds__GetDeviceInformationResponse->SerialNumber =  "SN2020000010";
    tds__GetDeviceInformationResponse->HardwareId =  "HV20201010";
    tds__GetDeviceInformationResponse->FirmwareVersion =  "SV20201010";
    FUN_OUT();
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetWsdlUrl(struct soap* soap, struct _tds__GetWsdlUrl *tds__GetWsdlUrl, struct _tds__GetWsdlUrlResponse *tds__GetWsdlUrlResponse)
{
    ...
    /*-----------------------------------------------------------------------------
     *  请求端口斿<GetWsdlUrl xmlns="http://www.onvif.org/ver10/device/wsdl" />
     *  赋值http://www.onvif.org/ver10/media/wsdl 可以轿http://www.baidu.com不能轿
     *  查明是以wsdl为关键字进行判别
     *-----------------------------------------------------------------------------*/
    ...
    switch(*tds__GetCapabilities->Category)
    {
        ...
        case tt__CapabilityCategory__Device:
            ...
            *tds__GetCapabilitiesResponse->Capabilities->Device->Network->Extension->Dot11Configuration = xsd__boolean__false_;
            ...
            /* :TODO:2014/6/23 16:55:43:Sean:  ploto:没有用但是没有设置为NULL会导腿挂掉*/
#if 1
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetHostname(struct soap* soap, struct _tds__GetHostname *tds__GetHostname, struct _tds__GetHostnameResponse *tds__GetHostnameResponse)
{
    ...
    else
    {
        ...
        /*-----------------------------------------------------------------------------
         *  检板buffer是否含有 '_' ' ' [0-9]非法hostname字符
         *-----------------------------------------------------------------------------*/
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__SetHostname(struct soap* soap, struct _tds__SetHostname *tds__SetHostname, struct _tds__SetHostnameResponse *tds__SetHostnameResponse)
{
    ...
    /*-----------------------------------------------------------------------------
     *  检板buffer是否含有 '_' ' ' [0-9]等非法onvif hostname字符
     *-----------------------------------------------------------------------------*/
...
//    strcpy(soap->header->wsa__MessageID,_HwId+4);
 /* :TODO:End---  */
    ...
    while(name[i]!=NULL)
    {
        ...
        tds__GetNetworkInterfacesResponse->NetworkInterfaces[i].Enabled = xsd__boolean__true_;/*Sean Hou: 实时检查网卡然后赋便*/
...
{
    struct sockaddr_in addr,mask; //add mask by duke for fix mask replace addr 2020.7.2
    int i=0;
...
#if 1    //edit by duke 2020.7.1
			 if (0)    //0 != gNetworkAdapterConfiguration.set_ip(ETHERNET_ADAPT, (struct sockaddr *)&addr))
                {
                    goto err;   
                }
                write_profile_string("ipaddr","address",tds__SetNetworkInterfaces->NetworkInterface->IPv4->Manual->Address,NET_CONF);
#else   
                /*-----------------------------------------------------------------------------
                ...
                if (0 != gNetworkAdapterConfiguration.get_netmask(ETHERNET_ADAPT, (struct sockaddr *)&mask))
                {
                    goto err;
                }                 
...
//                }                
                ...
                write_profile_string("ipaddr","address",tds__SetNetworkInterfaces->NetworkInterface->IPv4->Manual->Address,NET_CONF);
                if (0 != gNetworkAdapterConfiguration.set_netmask(ETHERNET_ADAPT, (struct sockaddr *)&mask))
                {
                    goto err;
                }
...
//                }               
        ...
        else
        {
 		  ...
 		  remove(NET_CONF);//add by duke 2020.7.2
        }
    }
...
#else
     //add by duke 2020.7.2 for set gateway
    write_profile_string("ipaddr","gw", tds__SetNetworkDefaultGateway->IPv4Address[0],NET_CONF);   
#endif
    FUN_OUT();
    return SOAP_OK;
