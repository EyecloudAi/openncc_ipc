...
SOAP_FMAC5 int SOAP_FMAC6 __trt__GetStreamUri(struct soap* soap, struct _trt__GetStreamUri *trt__GetStreamUri, struct _trt__GetStreamUriResponse *trt__GetStreamUriResponse)
{
    ...
    if(strcmp(trt__GetStreamUri->ProfileToken,"MainStreamProfileToken") == 0){
        sprintf(_IPAddr, "rtsp://%s:8554/liveRGB", inet_ntoa(*((struct in_addr *)&localIp)));//mainstream channel1 url
    }
    else if(strcmp(trt__GetStreamUri->ProfileToken,"SecondStreamProfileToken") == 0){
        sprintf(_IPAddr, "rtsp://%s:8554/liveRGB", inet_ntoa(*((struct in_addr *)&localIp)));//secondstream channel2 url
    }
    else //default
    {
    	sprintf(_IPAddr, "rtsp://%s:8554/liveRGB", inet_ntoa(*((struct in_addr *)&localIp)));
    }
    printf("[%d]ProfileToken=%s  URL ==== %s \n", __LINE__, trt__GetStreamUri->ProfileToken,_IPAddr);
    /* Response */
