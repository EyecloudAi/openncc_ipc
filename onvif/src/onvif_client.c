...
#endif
    header.wsa__MessageID = "123123123123" ;//guid_string; 
...
#include "onvif_cmd.h"
onvif_info ovinfo;
int open_ipc(int stream_index)
{  	
	int iRes=0;
	memset(&ovinfo,0,sizeof(ovinfo));
	strcpy(ovinfo.stUserInfo.username,"admin");
	strcpy(ovinfo.stUserInfo.password,"admin12345");

	//组播接口   
	if (ONVIF_ClientDiscovery(&ovinfo) != 0 )  
	{  
		printf("discovery failed!\n");
		return -1;
	}  

	if (ONVIF_Capabilities(&ovinfo,ovinfo.server_addr) != 0 )  
	{  
		printf("discovery failed!\n");
		return -1;
	}  
	
	if ((iRes = ONVIF_Profiles(&ovinfo,ovinfo.media_addr)) != 0 )  ///"http://192.168.1.103/onvif/Media"
	{  
		printf("get profiles failed! code : %d\n", iRes);
		return -1;
	}
	
	if ((iRes = ONVIF_StreamURL(&ovinfo,ovinfo.media_addr, ovinfo.media_info[stream_index].token,stream_index)) != 0 )
	{  
		printf("get streamURL failed! code : %d\n", iRes);
		return -1;
	}		
