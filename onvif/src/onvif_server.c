...
static void* soap_request_handler(void *argPtr)
{
    ...
    ServerSoap.bind_flags = SO_REUSEADDR;  
    if(!soap_valid_socket(soap_bind(&ServerSoap, NULL, ONVIF_REQ_LISTEN_PORT, 5)))
	{
...
int init_env()
{
    ...
    const char *scope6 = "onvif://www.onvif.org/location/HangZhou";
...
static bool isValidIP(const char *ipstr) {
    if (!ipstr) return false;
    int ip[4];
    int tmp = sscanf(ipstr, "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2], &ip[3]);

    if (4 != tmp) return false;
   // printf("inet_addr:%d,%d,%d,%d,%d\n", inet_addr(ipstr), &ip[0], &ip[1], &ip[2], &ip[3]);
...
int main(int argc,char ** argv)
{
    ...
    bool bStaticip=(access( NET_CONF, F_OK )== 0);
    
   //add by duke 2020.5.25 for check inet ok
    while (!bStaticip)  //(cntBind++<100) //add ip check valid
    {
	    	gNetworkAdapterConfiguration.get_ip(ETHERNET_ADAPT, &addr);
	    	char* self_ip=inet_ntoa((*(struct sockaddr_in *)&addr).sin_addr);
	    	//if (gNetworkAdapterConfiguration.is_ipaddr(self_ip,IPV4))
	    	if (isValidIP(self_ip))
	    	{
	    			PRINT_DBG("get valid ip %s \n",self_ip);
	    			break;
	    	}
	    	else
	    	{
	    			PRINT_DBG("get invalid ip %s ,check again\n",self_ip);
	    			sleep(2);
	    	}
    }
    //add end
    ...
    if (bStaticip) //find 
    	{
    		...
    		char gw[20];
    		char cmd[100];
    		if (read_profile_string("ipaddr","address",ipv4,sizeof(ipv4),"",NET_CONF))
    		{ 				
    				sprintf(cmd,"ifconfig %s %s",ETHERNET_ADAPT,ipv4);
    				PRINT_DBG("set ip: %s",cmd);
    				system(cmd);
    		}
    		
     	if (read_profile_string("ipaddr","gw",gw,sizeof(gw),"",NET_CONF))
    		{
    				sprintf(cmd,"route del default %s",ETHERNET_ADAPT);
    				system(cmd);
    				sprintf(cmd,"route add default gw %s %s",gw,ETHERNET_ADAPT);
    				PRINT_DBG("set gw: %s",cmd);
    				system(cmd);
    		}   		
    	}
  ...
  //  signal(SIGUSR1, hellobye_handle);  //delete by duke 2020.6.18
   // signal(SIGUSR2, hellobye_handle);
    ...
    while  (cntBind++<5)
    	{
	    if(!soap_valid_socket(soap_bind(soap_hello, NULL, ONVIF_LISTEN_PORT, 5)))
	    {
    ...
    soap_set_namespaces(&ServerSoap,  namespaces);
    ServerSoap.bind_flags = SO_REUSEADDR;  
    ServerSoap.connect_flags = SO_BROADCAST;  
    ServerSoap.omode = SOAP_IO_UDP;  
    ...
    while  (cntBind++<5)
    	{
	    if(!soap_valid_socket(soap_bind(&ServerSoap, NULL, ONVIF_LISTEN_PORT, 5)))
	    {
	        soap_print_fault(&ServerSoap, stderr);
	        sleep(2);
		 continue;	 
	   	...
	   	{
	   		PRINT_DBG("bind to %d success! %d \n",ONVIF_LISTEN_PORT,cntBind);
	   		break;
	   	}
    	}
    ...
    gNetworkAdapterConfiguration.get_ip(ETHERNET_ADAPT, &addr);
	char* self_ip=inet_ntoa((*(struct sockaddr_in *)&addr).sin_addr);
	PRINT_DBG("get valid mcast  ip %s \n",self_ip);
